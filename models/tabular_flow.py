...
class BasicBlock(nn.Module):
    ...
    def __init__(self, config, shape, latent_dim, type, input_dim=3, kernel1=3, kernel2=3, kernel3=3, init_zero=False):
        ...
        self.weight1 = nn.Parameter(
            torch.randn(input_dim * latent_dim, input_dim, kernel1) * 1e-5
        )
        ...
        self.weight2 = nn.Parameter(
            torch.randn(input_dim * latent_dim, input_dim * latent_dim, kernel2) * 1e-5
        ...
        self.weight3 = nn.Parameter(
            ...
            torch.randn(input_dim, input_dim * latent_dim, kernel3) * 1e-5
        )
    ...
    def forward(self, x):
        ...
        latent_output = F.conv1d(x, masked_weight1, bias=self.bias1, padding=self.padding1, stride=1)
        kernel_mid = masked_weight1.shape[-1] // 2
        # input_dim * latent_dim, input_dim, kernel1
        diag1 = torch.diagonal(
            masked_weight1[..., kernel_mid].view(self.latent_dim, self.input_dim, self.input_dim),
            dim1=-2, dim2=-1)  # shape: latent_dim x input_dim

        diag1 = self.non_linearity_derivative(latent_output). \
                    view(x.shape[0], self.latent_dim, self.input_dim, x.shape[-1]) \
                * diag1[None, :, :, None]  # shape: B x latent_dim x input_dim x img_shape x img_shape
        ...
        center1 = center1.view(self.latent_dim, self.input_dim, self.input_dim,
                               center1.shape[-1]).unsqueeze(0)
        # shape: latent_dim x 1 x input_dim x input_dim x kernel x kernel
        center3 = center3.view(self.input_dim, self.latent_dim, self.input_dim,
                               center3.shape[-1]).permute(1, 0, 2, 3).unsqueeze(1)
        ...
        center2 = center2.permute(0, 2, 1, 3, 4)
        center2 = sign_prods * torch.abs(center2)
        center2 = center2.permute(0, 2, 1, 3, 4).contiguous().view_as(self.weight2)
        masked_weight2 = (center2 * self.center_mask2 + self.weight2 * (1. - self.center_mask2)) * self.mask2
        ## more flexible diagonal

        latent_output = F.conv1d(latent_output, masked_weight2, bias=self.bias2, padding=self.padding2, stride=1)

        kernel_mid = masked_weight2.shape[-1] // 2
        diag2 = masked_weight2[..., kernel_mid].view(self.latent_dim, self.input_dim, self.latent_dim,
                                                                     self.input_dim)
        diag2 = torch.diagonal(diag2.permute(0, 2, 1, 3), dim1=-2,
                               dim2=-1)  # shape: latent_dim x latent_dim x input_dim
        diag2 = diag2[None, :, :, :, None]  # shape: 1 x latent_dim x latent_dim x input_dim x 1 x 1
        ...
        latent_output = F.conv1d(latent_output, masked_weight3, bias=self.bias3, padding=self.padding3, stride=1)

        kernel_mid = masked_weight3.shape[-1] // 2
        diag3 = masked_weight3[..., kernel_mid].view(self.input_dim, self.latent_dim, self.input_dim)
        diag3 = torch.diagonal(diag3.permute(1, 0, 2), dim1=-2, dim2=-1)  # shape: latent_dim x input_dim
        diag3 = latent_output_derivative.view(x.shape[0], self.latent_dim, self.input_dim, x.shape[-1]) \
                * diag3[None, :, :, None]  # shape: B x latent_dim x input_dim x img_shape x img_shape
        ...
        log_det += torch.sum(torch.log(diag + t), dim=(1, 2))
...
class SpaceToDepth(nn.Module):
    ...
    def forward(self, x):
        ...
        output = input.reshape(batch_size, channel * self.block_size, -1)
        return output, x[1]
        ...
        for layer_num in range(self.n_layers):
            if layer_num in subsampling_anchors:
                ...
                if not u:
                    ...
                    u = True
                #s = 43
                self.layers.append(SpaceToDepth(s))
                channel *= s
                image_size = int(image_size / s)
                latent_size //= s
